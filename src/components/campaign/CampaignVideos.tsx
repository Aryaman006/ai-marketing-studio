...
import { Video, Loader2, Trash2, Download, Play, Pencil } from "lucide-react";
...
export default function CampaignVideos({ campaign }: { campaign: Campaign }) {
  const { user } = useAuth();
  ...
  const { data: videos, isLoading } = useQuery({
    queryKey: ["videos", user?.id, campaign.id],
    queryFn: async () => {
